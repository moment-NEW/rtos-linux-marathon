...
D4: 
考完试后复习大学物理。下载了一些达妙资料，以及在修复用于编译Linux的服务器。
D5: 
考完离散，摆烂了一天，玩了会新出的小游戏。  
D6： 
开始着手RT-Thread的STM32H723VGTx的BSP板层，由于了解到个新的IMU算法叫做Madgwick，并且我在RM中的工作大部分与IMU有关，遂选择在达妙喵板（全称DM-MC-Board02）上部署SPI和BMI088的BSP层，作为简单的初始化驱动，但是发现H7相当复杂，正在准备中，目前就配个时钟树吧。
D7：  
这是本周计划的结束。今天没做什么，因为大部分时间在复习大物。  [text](c:/Users/18380/Downloads/STM32系列BSP制作教程.md)
- 下周计划：
- 需要协助/决策：
- 备注（链接/日志/截图）：
